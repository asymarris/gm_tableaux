...
picture tabvar(string  var="x",
  ...
  add(pic,new void(picture pic2, transform t) {
     for(int i=0; i<relier.length-1; ++i){ 
		  ...
		  path flec=point(im[i],d,t)--point(im[i+1],-d,t);
          draw(pic2,flec,stylefleche,Arrow(SimpleHead,size=8*linewidth(stylefleche)));
		  if(tvi[i]!=-1){
		      pair zzy=point(flec,.5), 
			       zzx=extension(zzy,zzy+(0,1),t*(0,-y1/2),t*(1,-y1/2));
			  draw(pic2,zzx--zzy,dotted);
			  label(pic2,yvi[i],zzy,Fill(white));
		      label(pic2,"$"+xvi[i]+"$",zzx,Fill(white));
		  }
          }
       }
  });
